 ...
 *  public void stop(int width, int height)
 *  buildTicker
 *  класс - Animator
 * @version 1.1 by budukh-rn
 *  
 */
public class InfiniteProgressPanel extends JComponent implements MouseListener
{
    ...
    public void start(int width, int height)
    {
        ...
        setVisible(true);
        ticker = buildTicker(width, height);
        animation = new Thread(new Animator(true,width, height));
        animation.start();
    }
     ...
     public void start()
    {
     ...
     public void stop(int width, int height)
     {
         if (animation != null) {
 	        animation.interrupt();
 	        animation = null;
 	        animation = new Thread(new Animator(false,width,height));
 	        animation.start();
         }
     }
    /**
        ...
        if (started)
        {
            ...
            for (int i = 0; i < ticker.length; i++)
            {
               // int channel = 224 - 128 / (i + 1);
            	...
            	int channelR = 224 - 124 / (i + 1);
            	int channelG = 224 - 39 / (i + 1);
            	int channelB = 255 - 4 / (i + 1);
            	
//            	int channelR = 224 - 180 / (i + 1);
//            	int channelG = 224 - 64 / (i + 1);
//            	int channelB = 224 + 25 / (i + 1);
            	
            //	System.out.println("budukh-rn-----28.03.2008-----channelR "+channelR);            	
     ...
     * 27.03.2008
     */
    private Area[] buildTicker(int width, int height)
    {
    ...
    private class Animator implements Runnable
    {
        ...
        protected Animator(boolean rampUp, int width, int height)
        {
            this.rampUp = rampUp;
            center = new Point2D.Double((double) width / 2, (double) height / 2);
        }
